...
export function PostDetailScreen({ route, navigation }: { route: any; navigation: any }) {
  ...
  const commentMutation = useMutation({
    mutationFn: () => communityApi.addComment(post.id, { authorId: user!.id, content: text.trim() }),
    onSuccess: () => {
      setText('')
      ...
      <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.postCard}>
          ...
          <View style={styles.postFooter}>
            <TouchableOpacity style={styles.actionBtn} onPress={() => likeMutation.mutate()} hitSlop={6}>
              <Ionicons
            ...
            comments.map((c) => (
              <View key={c.id} style={styles.commentRow}>
                <View style={styles.commentAvatar}>
                  <Ionicons name="person" size={14} color={colors.primary700} />
                </View>
                <View style={styles.commentBody}>
                  <Text style={styles.commentAuthor}>{c.author?.playerProfile?.displayName || 'Anónimo'}</Text>
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1 },
  postCard: { padding: 16, borderBottomWidth: 8, borderBottomColor: '#f9fafb' },
  ...
  postContent: { fontSize: 15, color: '#374151', lineHeight: 21, marginBottom: 12 },
  ...
  commentsTitle: { fontSize: 14, fontWeight: '700', color: colors.gray700, marginBottom: 12 },
  emptyComments: { fontSize: 13, color: colors.gray400, textAlign: 'center', marginTop: 12 },
  commentRow: { flexDirection: 'row', gap: 10, marginBottom: 16 },
  commentAvatar: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#d1fae5', alignItems: 'center', justifyContent: 'center' },
  commentBody: { flex: 1, backgroundColor: '#f9fafb', borderRadius: 12, padding: 10 },
  commentAuthor: { fontSize: 12, fontWeight: '700', color: '#111827', marginBottom: 2 },
  commentText: { fontSize: 13, color: '#374151', lineHeight: 18 },
  inputBar: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, borderTopWidth: 1, borderTopColor: colors.gray100 },
  commentInput: { flex: 1, borderWidth: 1.5, borderColor: '#e5e7eb', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 9, fontSize: 14, color: '#111827' },
  sendBtn: { width: 38, height: 38, borderRadius: 19, backgroundColor: colors.primary600, alignItems: 'center', justifyContent: 'center' },
