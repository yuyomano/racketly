...
export const clubsApi = {
  ...
  getAvailability: (id: string, date: string) => api.get(`/api/clubs/${id}/availability`, { params: { date } }),
}
...
export const bookingsApi = {
  ...
  submitMatch: (bookingId: string, data: { team1: string[]; team2: string[]; sets: { player1: number; player2: number }[] }) =>
...
export const membershipsApi = {
  ...
  subscribe:     (data: { userId: string; planId: string }) => api.post('/api/memberships/subscribe', data),
...
export const tournamentsApi = {
  ...
  withdraw: (id: string, participantId: string) => api.delete(`/api/tournaments/${id}/participants/${participantId}`),
...
export const classesApi = {
  list: (clubId: string, params?: object) => api.get(`/api/classes/${clubId}`, { params: { upcoming: '1', ...params } }),
  book: (slotId: string, data: { studentUserId: string; studentName: string; clubId?: string; pay?: boolean }) =>
