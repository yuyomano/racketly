...
type MyEnrollment = {
  id: string; progressPercent: number
  course: { id: string; title: string; sport: string; level: string; instructor: { displayName: string }; _count: { lessons: number } }
}
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          action={
            <Link href="/academy" className="text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl px-5 py-2.5 transition-colors">
              {t('emptyAction')}
            </Link>
          }
        />
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {t('instructorLessonsLabel', { instructor: e.course.instructor?.displayName ?? '', lessons: t('lessonsCount', { count: e.course._count.lessons }) })}
                    </p>
                  </div>
                  <span className="text-sm font-semibold text-emerald-600 shrink-0">{e.progressPercent}%</span>
                </div>
                <div className="h-1.5 bg-gray-100 rounded-full mt-3 overflow-hidden">
                  <div className="h-full bg-emerald-500" style={{ width: `${e.progressPercent}%` }} />
                </div>
              </Card>
            </Link>
          ))}
