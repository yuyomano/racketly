...
type PlayerResult = { id: string; name: string; city?: string }
...
type Player = {
  ...
  name: string
  isOwner: boolean
  paymentStatus?: 'pending' | 'paid' | 'courtesy'
  amountOwed?: number
...
type Booking = {
  ...
  slot: {
    ...
    startTime: string
    endTime: string
    court: { name: string; capacity?: number; club: { id: string; name: string; city: string } }
  }
}
...
// para los jugadores nuevos igual que al crear la reserva. El selector (filtros + búsqueda +
// invitados) replica el de "reserva nueva" (ClubBookingClient) para mantener la misma UX.
function EditPlayersModal({
  ...
  const owner = booking.players?.find((p) => p.isOwner)
  const [players, setPlayers] = useState<{ id: string; name: string; isGuest?: boolean }[]>(
    (booking.players ?? [])
      .filter((p) => !p.isOwner)
      .map((p) => ({ id: p.userId || p.guestId!, name: p.name, isGuest: !p.userId }))
  )
  const [showAddPlayer, setShowAddPlayer] = useState(false)
  const [filter, setFilter] = useState<PlayerFilter>('club')
  ...
  const { data: results, isFetching } = useQuery({
    queryKey: ['user-search', query, filter, booking.slot.court.club.id, userId],
    queryFn: async () => {
      ...
      if (!res.ok) return []
      const data = await res.json()
      return (data.data ?? []) as PlayerResult[]
    },
    enabled: filter !== 'all' || query.trim().length >= 2,
  })

  const capacity = booking.slot.court.capacity || 4
  ...
  function addGuest() {
    ...
    if (!name || players.length + 1 >= capacity) return
    setPlayers((prev) => [
      ...prev,
      { id: `guest_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`, name, isGuest: true },
    ])
    setGuestName('')
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-ink-50 rounded-xl px-3 py-2.5 space-y-1.5 text-xs">
        <div className="flex items-center justify-between text-ink-500">
          <span>{booking.slot.court.name}</span>
          <span>
        ...
        <div className="flex items-center justify-between text-ink-500">
          <span>{t('editTotalLabel')}</span>
          <span className="font-semibold text-ink-800">
            {formatCurrency(booking.amountPaid ?? 0, booking.currency)}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        {players.map((p) => {
          const existing = booking.players?.find((x) => (x.userId || x.guestId) === p.id)
          return (
            <div
              key={p.id}
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {existing?.coveredBy ? (
                  ...
                  PAYMENT_ICON[existing.paymentStatus ?? 'pending']
                ) : null}
                <button
      ...
      >
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 border border-ink-200 rounded-xl px-3 py-2">
            ...
            />
            {isFetching && <Loader2 className="w-4 h-4 animate-spin text-ink-300" />}
          </div>
          ...
          <div className="space-y-1 h-72 overflow-y-auto">
            ...
            ) : (results ?? []).filter((r) => !players.find((x) => x.id === r.id)).length === 0 ? (
              <p className="text-xs text-ink-400 text-center py-4">
                {isFetching ? t('playerSearching') : t('playerSearchEmpty')}
              </p>
            ) : (
              (results ?? [])
                .filter((r) => !players.find((x) => x.id === r.id))
                .map((r) => (
                  <button
                    key={r.id}
                    onClick={() => addPlayer(r)}
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      {r.city && <p className="text-xs text-ink-400 truncate">{r.city}</p>}
                    </div>
                  </button>
                ))
            )}
