...
export default function LoginPage() {
  ...
  const DEMO_ACCOUNTS = [
    { label: t('demoAdminLabel'), sub: t('demoAdminSub'), email: 'admin@racketly.app', pass: 'Admin2026!' },
    { label: t('demoGroupALabel'), sub: t('demoGroupASub'), email: 'carlos.pro@racketly.app', pass: 'Test2026!' },
    { label: t('demoGroupBLabel'), sub: t('demoGroupBSub'), email: 'juan.amateur@racketly.app', pass: 'Test2026!' },
  ]
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error || t('errorLogin')); return }
  ...
  return (
    <div className="min-h-screen bg-[#042b22] flex items-center justify-center px-4 relative overflow-hidden">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-2xl space-y-5">
          <div className="flex border border-gray-200 rounded-xl overflow-hidden">
            <button type="button" onClick={() => setMode('player')}
              className={cn('flex-1 py-2.5 text-sm font-semibold transition-colors', mode === 'player' ? 'bg-emerald-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50')}
            >{t('tabPlayer')}</button>
            <button type="button" onClick={() => setMode('admin')}
              className={cn('flex-1 py-2.5 text-sm font-semibold transition-colors', mode === 'admin' ? 'bg-emerald-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50')}
            >{t('tabAdmin')}</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>{mode === 'admin' ? t('submitAdmin') : t('submitPlayer')} <ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>
          ...
          {mode === 'player' && (
            <p className="text-center text-sm text-gray-400">
              {t('noAccount')}{' '}
              <Link href="/registro" className="text-emerald-600 font-semibold hover:text-emerald-700">{t('signupLink')}</Link>
            </p>
          )}

          {mode === 'admin' && (
          <div className="border-t border-gray-100 pt-4">
            <p className="text-xs font-semibold text-gray-400 mb-2.5">{t('demoAccountsLabel')}</p>
            <div className="grid grid-cols-3 gap-2">
              {DEMO_ACCOUNTS.map((acc) => (
                <button
                  ...
                  className="text-left bg-gray-50 hover:bg-emerald-50 border border-gray-100 hover:border-emerald-200 rounded-xl px-3 py-2.5 transition-colors"
                >
                  <p className="text-[11px] font-bold text-gray-700">{acc.label}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">{acc.sub}</p>
                </button>
              ))}
            </div>
          </div>
