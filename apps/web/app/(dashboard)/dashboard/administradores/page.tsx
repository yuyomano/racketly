...
function getCurrentUserId(): string {
  try {
    const raw = document.cookie.split('; ').find((c) => c.startsWith('racketly_user='))?.split('=').slice(1).join('=')
    if (!raw) return ''
...
export default function AdministradoresPage() {
  ...
  const loadData = useCallback(async (cId: string) => {
    ...
    try {
      ...
      if (adminsRes.ok) {
        ...
        if (me?.role === 'owner') {
          const invRes = await fetch(`${GW}/api/clubs/${cId}/invitations`, { credentials: 'include' })
          if (invRes.ok) {
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setInvError(data.error || t('invite.genericError')); return }
    ...
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        {clubName && (
          <p className="text-sm text-gray-400 mt-0.5">
            {clubName}
            {myRole && (
              <span className={`ml-2 inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${isOwner ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>
      ...
      {isOwner && (
        <Card className="p-6">
          <h3 className="font-semibold text-gray-900 mb-1 text-sm flex items-center gap-2">
            <Mail className="w-4 h-4 text-emerald-600" /> {t('invite.title')}
          </h3>
          ...
          <form onSubmit={handleInvite} className="flex gap-3">
            <Input
              ...
              placeholder={t('invite.emailPlaceholder')} required
              className="flex-1"
            />
            <Select
              value={invRole}
          ...
          {invError   && <p className="text-red-600 text-sm mt-2">{invError}</p>}
          {invSuccess && <p className="text-emerald-600 text-sm mt-2">{invSuccess}</p>}
        </Card>
      )}
      ...
      {isOwner && invitations.length > 0 && (
        <Card className="overflow-hidden">
          <CardHeader>
            <CardTitle>
              <Clock className="w-4 h-4 text-amber-500 inline mr-1.5" />
                ...
                <div key={inv.id} className="flex items-center gap-4 px-6 py-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-xs mt-0.5 ${expired ? 'text-red-500' : 'text-gray-400'}`}>
                      {expired ? t('pendingInvitations.expired') : t('pendingInvitations.expiresOn', { date: new Date(inv.expiresAt).toLocaleDateString(locale) })}
                    </p>
                  </div>
      ...
      {/* Lista de admins */}
      <Card className="overflow-hidden">
        <CardHeader><CardTitle>{t('team.title', { count: admins.length })}</CardTitle></CardHeader>
        {loading ? (
                ...
                <div key={a.id} className="flex items-center gap-4 px-6 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-gray-900 text-sm">
                      {name}
                      {isMe && <span className="ml-2 text-xs text-gray-400 font-normal">{t('team.you')}</span>}
                    </p>
