...
import { CircleDot, Lightbulb, Settings, X, Plus, Zap, RefreshCw, Loader2, Clock, Wrench, Trash2 } from 'lucide-react'
...
type Court = {
  ...
  surface: string; isIndoor: boolean; hasLighting: boolean
  basePrice: number; peakPrice: number; currency: string
...
type ClubConfig = {
  bookingHorizonDays: number; currency: string; slotGenerationHour: number; timezone: string
  paymentWarningMinutesBefore: number; rosterWarningHoursBefore: number
}

type MaintenanceBlock = { id: string; courtId: string; startAt: string; endAt: string; description: string }
...
function CourtSettingsModal({ court, onClose, onSaved }: {
  court: Court; onClose: () => void; onSaved: (c: Court) => void
}) {
  ...
  async function handleSave(andGenerate = false) {
    ...
    try {
      const res = await fetch(`${GW}/api/courts/${court.id}`, {
        ...
        body: JSON.stringify({ ...form, slotDuration: Number(form.slotDuration), generateSlots: andGenerate }),
      })
      const data = await res.json()
      if (!res.ok) { showToast(data.error ?? t('settingsErrorSaveToast'), false); return }
      onSaved(data.data)
      showToast(
        andGenerate
  ...
  return (
    <Modal open onClose={onClose} maxWidth="lg">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-bold text-gray-900">{t('settingsModalTitle')}</h2>
            <p className="text-sm text-gray-400 mt-0.5">{court.name}</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
        </div>

        {toast && (
          <div className={`text-sm px-4 py-2.5 rounded-xl mb-4 ${toast.ok ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
            {toast.msg}
          </div>
        )}
        ...
        <div className="space-y-5">
          ...
          {/* Duración del slot */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">{t('slotDurationLabel')}</label>
          ...
          <div className="bg-gray-50 rounded-2xl p-4 space-y-3">
            <p className="text-xs font-bold text-gray-600 uppercase tracking-wide">{t('weekdaySectionLabel')}</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1">{t('openingLabel')}</label>
                <TimeSelect value={form.openTimeWeekday} onChange={(v) => setForm({ ...form, openTimeWeekday: v })} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1">{t('closingLabel')}</label>
                <TimeSelect value={form.closeTimeWeekday} onChange={(v) => setForm({ ...form, closeTimeWeekday: v })} />
              </div>
            </div>
            <p className="text-[10px] text-gray-400">
              {t('slotsPerDay', { count: previewSlots(form.openTimeWeekday, form.closeTimeWeekday, form.slotDuration).length })}
            </p>
          </div>

          {/* Horarios Sábado–Domingo */}
          <div className="bg-blue-50 rounded-2xl p-4 space-y-3">
            <p className="text-xs font-bold text-blue-600 uppercase tracking-wide">{t('weekendSectionLabel')}</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-blue-500 mb-1">{t('openingLabel')}</label>
                <TimeSelect value={form.openTimeWeekend} onChange={(v) => setForm({ ...form, openTimeWeekend: v })} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-blue-500 mb-1">{t('closingLabel')}</label>
                <TimeSelect value={form.closeTimeWeekend} onChange={(v) => setForm({ ...form, closeTimeWeekend: v })} />
              </div>
            </div>
            <p className="text-[10px] text-blue-400">
              {t('slotsPerDay', { count: previewSlots(form.openTimeWeekend, form.closeTimeWeekend, form.slotDuration).length })}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">
                {t('offPeakPriceLabel')} <span className="font-normal text-gray-400">({court.currency})</span>
              </label>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">
                {t('peakPriceLabel')} <span className="font-normal text-gray-400">({court.currency})</span>
              </label>
              <Input type="number" min={0} value={form.peakPrice}
          ...
          {preview.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-2">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{t('previewLabel')}</p>
                  ...
                  {(['weekday', 'weekend'] as const).map((d) => (
                    <button key={d} type="button"
                      ...
                      className={`px-2 py-0.5 font-semibold transition-colors ${
                        previewDay === d ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-50'
                      }`}
                    >
                ...
                <span className="text-[10px] text-gray-400">{t('slotsPerDay', { count: preview.length })}</span>
              </div>
              <div className="flex flex-wrap gap-1 max-h-24 overflow-y-auto">
                {preview.map((s) => (
                  <span key={s.start}
                    className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${
                      s.isPeak ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
                    }`}
                  >
                    {s.start}
                  </span>
              ...
              <p className="text-[10px] text-gray-400 mt-1">{t('peakLegend')}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-3 pt-6">
          ...
          <Button type="button" variant="secondary" onClick={() => handleSave(false)}
            disabled={saving || generating} className="flex-1">
...
function AddCourtModal({ clubId, clubCurrency, onClose, onCreated }: {
  clubId: string; clubCurrency: string; onClose: () => void; onCreated: (c: Court) => void
}) {
  const t = useTranslations('Canchas')
  const [form, setForm] = useState({
    name: '', sport: 'padel', surface: 'cristal',
    isIndoor: false, hasLighting: true, capacity: 4,
    basePrice: 0, peakPrice: 0, currency: clubCurrency,
    openTimeWeekday: '07:00', closeTimeWeekday: '23:00',
    openTimeWeekend: '07:00', closeTimeWeekend: '23:00',
    slotDuration: 60,
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.name.trim()) { setError(t('addNameRequiredError')); return }
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setError(data.error || t('addErrorCreate')); return }
      onCreated(data.data)
    } catch {
    ...
    <Modal open onClose={onClose} title={t('addModalTitle')} maxWidth="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">{t('surfaceLabel')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">{t('capacityLabel')}</label>
              <Input type="number" min={2} max={8} value={form.capacity}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">{t('priceOffPeakLabel')}</label>
              <Input type="number" min={0} value={form.basePrice}
                onChange={(e) => setForm({ ...form, basePrice: Number(e.target.value) })} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">{t('pricePeakLabel')}</label>
              <Input type="number" min={0} value={form.peakPrice}
                onChange={(e) => setForm({ ...form, peakPrice: Number(e.target.value) })} />
            </div>
          </div>
          ...
          <div className="flex gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.isIndoor}
                onChange={(e) => setForm({ ...form, isIndoor: e.target.checked })}
                className="w-4 h-4 accent-emerald-600" />
              <span className="text-sm font-medium text-gray-700">{t('indoorLabel')}</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.hasLighting}
                onChange={(e) => setForm({ ...form, hasLighting: e.target.checked })}
                className="w-4 h-4 accent-emerald-600" />
              <span className="text-sm font-medium text-gray-700">{t('lightingLabel')}</span>
            </label>
          </div>
...
function MaintenanceModal({ court, onClose }: { court: Court; onClose: () => void }) {
  ...
  useEffect(() => { loadBlocks() }, [court.id]) // eslint-disable-line react-hooks/exhaustive-deps

  async function handleCreate(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!form.description.trim()) { setError(t('maintenanceDescRequiredError')); return }
    ...
    if (new Date(endAtIso) <= new Date(startAtIso)) { setError(t('maintenanceEndBeforeStartError')); return }
    ...
    try {
      const res = await fetch(`${GW}/api/courts/${court.id}/maintenance`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ startAt: startAtIso, endAt: endAtIso, description: form.description.trim() }),
      })
      const data = await res.json()
      if (!res.ok) { setError(data.error || t('maintenanceCreateError')); return }
  ...
  async function handleDelete(blockId: string) {
    ...
    try {
      const res = await fetch(`${GW}/api/courts/${court.id}/maintenance/${blockId}`, { method: 'DELETE' })
      if (!res.ok) { showToast(t('maintenanceDeleteError'), false); return }
  ...
  return (
    <Modal open onClose={onClose} maxWidth="lg">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-gray-900">{t('maintenanceModalTitle')}</h2>
          <p className="text-sm text-gray-400 mt-0.5">{court.name}</p>
        </div>
        <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
      </div>
      ...
      <form onSubmit={handleCreate} className="space-y-4 bg-gray-50 rounded-2xl p-4">
        {error && <p className="text-red-600 text-sm bg-red-50 rounded-xl px-4 py-2">{error}</p>}

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">{t('maintenanceStartDateLabel')}</label>
            <Input type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">{t('maintenanceStartTimeLabel')}</label>
            <TimeSelect value={form.startTime} onChange={(v) => setForm({ ...form, startTime: v })} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">{t('maintenanceEndDateLabel')}</label>
            <Input type="date" value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">{t('maintenanceEndTimeLabel')}</label>
            <TimeSelect value={form.endTime} onChange={(v) => setForm({ ...form, endTime: v })} />
          </div>
        </div>

        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-1">{t('maintenanceDescLabel')}</label>
          <textarea
            value={form.description}
            onChange={(e) => setForm({ ...form, description: e.target.value })}
            placeholder={t('maintenanceDescPlaceholder')}
            rows={2}
            className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500"
          />
        </div>
      ...
      <div className="mt-5">
        <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">{t('maintenanceUpcomingTitle')}</p>
        {loading ? (
          <p className="text-sm text-gray-400">{t('maintenanceLoading')}</p>
        ) : blocks.length === 0 ? (
          <p className="text-sm text-gray-400">{t('maintenanceEmpty')}</p>
        ) : (
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {blocks.map((b) => (
              <div key={b.id} className="flex items-start justify-between gap-3 bg-white border border-gray-100 rounded-xl px-4 py-3">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-gray-800">
                    {new Date(b.startAt).toLocaleString(locale, { dateStyle: 'medium', timeStyle: 'short', timeZone: 'UTC' })}
                    {' → '}
                    {new Date(b.endAt).toLocaleString(locale, { dateStyle: 'medium', timeStyle: 'short', timeZone: 'UTC' })}
                  </p>
                  <p className="text-xs text-gray-500 mt-0.5 break-words">{b.description}</p>
                </div>
                <button
...
function CourtCard({ court, onToggle, onSettings, onMaintenance }: {
  court: Court
  ...
  const t = useTranslations('Canchas')
  const slotsWD = previewSlots(court.openTimeWeekday || '07:00', court.closeTimeWeekday || '23:00', court.slotDuration || 60)
  const slotsWE = previewSlots(court.openTimeWeekend || '07:00', court.closeTimeWeekend || '23:00', court.slotDuration || 60)
    ...
    <Card className={`transition-all ${!court.isActive ? 'opacity-60' : ''}`}>
      ...
      <div className="p-5 flex items-start justify-between">
        <div className="flex items-start gap-3 min-w-0">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${court.sport === 'padel' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
              ...
              {court.hasLighting && <Badge tone="amber"><Lightbulb className="w-3 h-3" /> {t('cardLightedBadge')}</Badge>}
              <Badge tone="gray">{t('cardPlayersBadge', { count: court.capacity })}</Badge>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0 ml-2">
          <button onClick={() => onMaintenance(court)}
            className="p-1.5 rounded-lg text-gray-400 hover:text-amber-600 hover:bg-amber-50 transition-colors"
            title={t('cardMaintenanceTooltip')}>
            <Wrench className="w-4 h-4" />
          </button>
          <button onClick={() => onSettings(court)}
            className="p-1.5 rounded-lg text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors"
            title={t('cardSettingsTooltip')}>
            <Settings className="w-4 h-4" />
          </button>
          <button onClick={() => onToggle(court.id)}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              court.isActive ? 'bg-emerald-500' : 'bg-gray-300'
            }`}>
      ...
      {/* Precios */}
      <div className="px-5 py-3 flex gap-4 text-sm border-t border-gray-50">
        <div>
          <p className="text-xs text-gray-400">{t('cardOffPeak')}</p>
          <p className="font-bold text-gray-900">{court.currency} {(court.basePrice ?? 0).toLocaleString()}</p>
        </div>
        <div>
          <p className="text-xs text-gray-400">{t('cardPeak')}</p>
          <p className="font-bold text-amber-600">{court.currency} {(court.peakPrice ?? 0).toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="px-5 pb-4 border-t border-gray-50 pt-3 space-y-2">
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">
          {t('cardPerSlot', { duration: court.slotDuration === 90 ? t('duration90m') : t('duration1h') })}
        </p>
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div>
            <p className="text-[10px] text-gray-400 mb-1">{t('cardWeekdaySlots', { count: slotsWD.length })}</p>
            <p className="text-gray-600 font-mono">
              {court.openTimeWeekday || '07:00'} – {court.closeTimeWeekday || '23:00'}
            </p>
          </div>
          <div>
            <p className="text-[10px] text-blue-400 mb-1">{t('cardWeekendSlots', { count: slotsWE.length })}</p>
        ...
        <div className="flex flex-wrap gap-1 max-h-12 overflow-hidden">
          {slotsWD.map((s) => (
            <span key={s.start} className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${
              s.isPeak ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
            }`}>
              {s.start}
            </span>
...
function ClubConfigCard({ clubId, config, onSaved }: {
  clubId: string
  config: ClubConfig
  ...
  async function saveHorizon() {
    ...
    try {
      ...
      if (!r.ok) { showToast(d.error ?? t('configErrorSaveToast'), false); return }
      onSaved({ ...config, bookingHorizonDays: days })
      showToast(t('configSavedToast'), true)
    } catch { showToast(t('configErrorConnectionToast'), false) }
  ...
  async function savePayWarn() {
    ...
    finally { setSavingPayWarn(false) }
  }
  ...
  async function saveRosterWarn() {
    ...
    finally { setSavingRosterWarn(false) }
  }
  ...
  async function generateAll() {
    ...
    try {
      ...
      const d = await r.json()
      if (!r.ok) { showToast(d.error ?? t('configErrorGenerateToast'), false); return }
      showToast(t('slotsGeneratedToast', { slots: d.data.slotsCreated, courts: d.data.courtsProcessed }), true)
    } catch { showToast(t('configErrorConnectionToast'), false) }
  ...
  return (
    <Card className="p-5">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex-1">
          <h3 className="text-sm font-bold text-gray-800 flex items-center gap-1.5">
            <Clock className="w-4 h-4 text-emerald-600" /> {t('horizonTitle')}
          </h3>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {toast && (
            <span className={`text-xs px-3 py-1 rounded-lg ${toast.ok ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
              {toast.msg}
            </span>
          )}
          <div className="flex items-center gap-2">
            ...
            >
              {[1, 3, 7, 14, 21, 30, 60].map((d) => (
                <option key={d} value={d}>{t('daysOption', { count: d })}</option>
              ))}
            </Select>
            <Button
              size="sm"
          ...
          <Button size="sm" onClick={generateAll} disabled={genning}>
            {genning ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5" />}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-4 pt-4 border-t border-gray-100">
        <div className="flex-1">
          <h3 className="text-sm font-bold text-gray-800 flex items-center gap-1.5">
            <Clock className="w-4 h-4 text-emerald-600" /> {t('autoGenTitle')}
          </h3>
          <p className="text-xs text-gray-400 mt-0.5">
            {t('autoGenDesc')}
          </p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-4 pt-4 border-t border-gray-100">
        <div className="flex-1">
          <h3 className="text-sm font-bold text-gray-800 flex items-center gap-1.5">
            <Clock className="w-4 h-4 text-emerald-600" /> {t('timezoneTitle')}
          </h3>
          <p className="text-xs text-gray-400 mt-0.5">
            {t('timezoneDesc')}
          </p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-4 pt-4 border-t border-gray-100">
        <div className="flex-1">
          <h3 className="text-sm font-bold text-gray-800 flex items-center gap-1.5">
            <Clock className="w-4 h-4 text-emerald-600" /> {t('paymentWarningTitle')}
          </h3>
          <p className="text-xs text-gray-400 mt-0.5">
            {t('paymentWarningDesc')}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Select
          ...
          >
            {[1, 2, 3, 5, 7, 10, 14].map((m) => (
              <option key={m} value={m}>{t('minutesOption', { count: m })}</option>
            ))}
          </Select>
          <Button
            size="sm"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mt-4 pt-4 border-t border-gray-100">
        <div className="flex-1">
          <h3 className="text-sm font-bold text-gray-800 flex items-center gap-1.5">
            <Clock className="w-4 h-4 text-emerald-600" /> {t('rosterWarningTitle')}
          </h3>
          <p className="text-xs text-gray-400 mt-0.5">
            {t('rosterWarningDesc')}
          </p>
...
export default function CanchasPage() {
  ...
  const [clubConfig,  setClubConfig]  = useState<ClubConfig>({
    bookingHorizonDays: 14, currency: 'COP', slotGenerationHour: 6, timezone: 'UTC',
    paymentWarningMinutesBefore: 5, rosterWarningHoursBefore: 6,
  })
  const [loading,     setLoading]     = useState(true)
  ...
  useEffect(() => {
    ...
    let cId = stored
    try { const p = JSON.parse(stored); if (p?.id) cId = p.id } catch { /* plain string */ }
    setClubId(cId)
    loadCourts(cId)
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-5 text-center">
          <p className="text-3xl font-black text-gray-900">{courts.length}</p>
          <p className="text-sm text-gray-500 mt-1">{t('statTotalCourts')}</p>
        </Card>
        <Card className="p-5 text-center">
          <p className="text-3xl font-black text-emerald-600">{active}</p>
          <p className="text-sm text-gray-500 mt-1">{t('statActiveCourts')}</p>
        </Card>
        <Card className="p-5 text-center">
          <p className="text-3xl font-black text-gray-400">{courts.length - active}</p>
          <p className="text-sm text-gray-500 mt-1">{t('statInactiveCourts')}</p>
        </Card>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Button onClick={() => { if (!clubId) { alert(t('selectClubAlert')); return }; setShowAdd(true) }}>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            icon={CircleDot}
            title={t('emptyTitle')}
            action={<Button size="sm" onClick={() => setShowAdd(true)}><Plus className="w-3.5 h-3.5" /> {t('addFirstCourt')}</Button>}
          />
        </Card>
      ) : (
      ...
      {showAdd && (
        <AddCourtModal
          ...
          onCreated={(court) => { setCourts((prev) => [...prev, court]); setShowAdd(false) }}
        />
      )}
