...
export default function ClasesPage() {
  ...
  async function cancelStudentBooking(slot: ClassSlot, booking: ClassBooking) {
    ...
    try {
      ...
      setSlots(
        slots.map((s) =>
          s.id === slot.id ? { ...s, bookings: s.bookings.filter((b) => b.id !== booking.id) } : s
        )
      )
      if (data.refunded)
        alert(
  ...
  const pendingRevenue = upcomingSlots.reduce(
    (sum, s) =>
      sum +
      s.bookings
        .filter((b) => b.status === 'active' && b.paymentStatus !== 'paid')
        .reduce((a, b) => a + (b.amountOwed - b.amountPaid), 0),
    0
  )
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard
          icon={GraduationCap}
          label={t('statActiveProfessors')}
          value={String(activeProfessors.length)}
        />
        <StatCard
          icon={CalendarDays}
          label={t('statUpcomingClasses')}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-ink-400">
            {t('pageSubtitle', { clubName: clubName || t('defaultClubName') })}
          </p>
        </div>
      ...
      <Card>
        <CardBody>
          <p className="text-sm font-bold text-ink-700 mb-3">{t('professorsSectionTitle')}</p>
          {professors.length === 0 ? (
            <p className="text-sm text-ink-400">{t('noProfessors')}</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {professors.map((p) => (
                <div
                  key={p.id}
                  className={`border border-ink-100 rounded-xl p-3 flex items-start justify-between gap-2 ${!p.isActive ? 'opacity-50' : ''}`}
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-ink-400 mt-0.5">
                      {t('professorRateLine', {
                        rate: formatCurrency(p.hourlyRate, p.currency),
                        sport: p.sport,
                      })}
                    </p>
      ...
      {loading ? (
        ...
        <>
          <div>
            <p className="text-sm font-bold text-ink-700 mb-3">{t('upcomingClassesTitle')}</p>
              ...
              <EmptyState
                ...
                action={
                  <Button
                    ...
                    disabled={activeProfessors.length === 0}
                  >
                    <Plus className="w-4 h-4 mr-1.5" /> {t('scheduleClassButton')}
                  </Button>
                }
              ...
              <div className="space-y-3">
                {upcomingSlots.map((s) => (
                  <SlotCard
                    ...
                    onMarkPaid={markPaid}
                    payingId={payingId}
                    onAddStudent={setAddStudentSlot}
                    onCancelStudent={cancelStudentBooking}
          ...
          {pastSlots.length > 0 && (
            <div>
              <p className="text-sm font-bold text-ink-700 mb-3">{t('pastClassesTitle')}</p>
              <div className="space-y-3">
      ...
      {(showProfModal || editingProf) && clubId && (
        <ProfessorModal
          ...
          onSaved={(p) => {
            setProfessors(
              editingProf ? professors.map((x) => (x.id === p.id ? p : x)) : [...professors, p]
            )
            setShowProfModal(false)
            setEditingProf(null)
          }}
      ...
      {addStudentSlot && clubId && (
        <AddStudentModal
          ...
          onSaved={(booking) => {
            setSlots(
              slots.map((s) =>
                s.id === addStudentSlot.id ? { ...s, bookings: [...s.bookings, booking] } : s
              )
            )
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="shrink-0 text-center bg-ink-50 border border-ink-100 rounded-xl px-3 py-1.5 w-[4.25rem]">
              ...
              <p className="text-lg font-black text-ink-900 leading-none mt-1">
                {dateObj.getDate()}
              </p>
              <p className="text-[0.6875rem] font-semibold text-court-600 mt-1 truncate">
                {slot.startTime}
              </p>
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="font-bold text-ink-900">{slot.professor.name}</p>
                <Badge tone={slot.professor.isExternal ? 'violet' : 'emerald'}>
                  {slot.professor.isExternal ? t('badgeExternal') : t('badgeClub')}
                </Badge>
              ...
              <div className="flex items-center gap-1.5 flex-wrap mt-1.5">
                <span className="text-xs text-ink-500 bg-ink-50 rounded-full px-2 py-0.5">
                  {t('durationPill', { minutes: slot.durationMinutes })}
                </span>
                {slot.court && (
                  <span className="text-xs text-ink-500 bg-ink-50 rounded-full px-2 py-0.5">
                    {t('courtPill', { court: slot.court.name })}
                  </span>
                )}
                <span className="text-xs text-ink-500 bg-ink-50 rounded-full px-2 py-0.5">
                  {t('pricePerStudentPill', { price: formatCurrency(slot.price, slot.currency) })}
                </span>
                <span
                  className={`text-xs rounded-full px-2 py-0.5 font-medium ${cuposFull ? 'bg-trophy-50 text-trophy-700' : 'bg-ink-50 text-ink-500'}`}
                >
                  {t('cuposPill', { active: activeBookings.length, max: slot.maxStudents })}
                </span>
              </div>
            </div>
          </div>
          {!past && slot.status !== 'cancelled' && (
            <div className="flex items-center gap-1.5 shrink-0">
              {activeBookings.length < slot.maxStudents && (
                <button
                  onClick={() => onAddStudent(slot)}
                  className="text-xs font-semibold text-court-600 hover:text-court-800 border border-court-200 hover:border-court-400 bg-court-50 rounded-lg px-2.5 py-1.5"
                >
                  {t('addStudentButton')}
                </button>
              )}
              <button
                onClick={() => onEdit(slot)}
                className="text-xs font-semibold text-ink-500 hover:text-ink-700 border border-ink-200 hover:border-ink-400 bg-ink-50 rounded-lg px-2.5 py-1.5"
              >
                {t('editButton')}
              </button>
              <button
                onClick={() => onCancel(slot)}
                className="text-xs font-semibold text-referee-500 hover:text-referee-700 border border-referee-100 hover:border-referee-400 bg-referee-50 rounded-lg px-2.5 py-1.5"
              >
                {t('cancelClassButton')}
              </button>
            </div>
              ...
              <div key={b.id} className="py-2 flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  {b.paymentStatus === 'paid' ? (
                    <Badge tone="emerald">
                      {b.paymentMethod === 'cash' ? t('paidCash') : t('paidCard')}
                    </Badge>
                  ) : (
                    <>
                      <button
...
function ProfessorModal({
  ...
  async function handleSave() {
    if (!form.isExternal && !selectedPlayer) {
      setError(t('errorSelectProfessorPlayer'))
      return
    }
    if (form.isExternal && !form.name.trim()) {
      setError(t('errorNameRequired'))
      return
    }
    if (form.hourlyRate < 0) {
      setError(t('errorRateNegative'))
      return
    }
    setSaving(true)
    setError('')
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={professor ? t('editProfessorTitle') : t('newProfessorTitle')}
      maxWidth="sm"
    >
      {error && (
      ...
      <div className="space-y-3">
        <div>
          <label className="block text-sm font-semibold text-ink-700 mb-1">{t('typeLabel')}</label>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setForm({ ...form, isExternal: false })}
              className={`flex-1 py-2 rounded-xl text-sm font-semibold border ${!form.isExternal ? 'bg-court-50 border-court-400 text-court-700' : 'border-ink-200 text-ink-500'}`}
            >
              {t('badgeClub')}
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-sm font-semibold border ${form.isExternal ? 'bg-trophy-50 border-trophy-400 text-trophy-700' : 'border-ink-200 text-ink-500'}`}
            >
              {t('badgeExternal')}
            </button>
          </div>
          ...
          <div>
            ...
            {selectedPlayer ? (
              ...
              <>
                ...
                />
                {searching && <p className="text-xs text-ink-400 mt-1">{t('searching')}</p>}
                {results.length > 0 && (
                  <div className="mt-1.5 border border-ink-200 rounded-xl divide-y divide-ink-100 max-h-40 overflow-y-auto">
                    {results.map((u) => (
                      <button
                        key={u.id}
                        onClick={() => {
                          setSelectedPlayer({ id: u.id, name: u.name })
                          setQuery('')
                          setResults([])
                        }}
            ...
            <p className="text-xs text-ink-400 mt-1">{t('playerMustBeRegistered')}</p>
          </div>
        )}

        <div>
          <label className="block text-sm font-semibold text-ink-700 mb-1">{t('phoneLabel')}</label>
        ...
        <div>
          <label className="block text-sm font-semibold text-ink-700 mb-1">{t('bioLabel')}</label>
          <Textarea
            value={form.bio}
...
function SlotModal({
  ...
  async function handleSave() {
    if (!form.professorId) {
      setError(t('errorSelectProfessor'))
      return
    }
    if (!form.date || !form.startTime) {
      setError(t('errorDateTimeRequired'))
      return
    }
    if (form.price < 0) {
      setError(t('errorPriceNegative'))
      return
    }
    setSaving(true)
    setError('')
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={slot ? t('editClassTitle') : t('scheduleClassTitle')}
      maxWidth="sm"
    >
      {error && (
        <p className="text-referee-600 text-sm bg-referee-50 rounded-xl px-3 py-2 mb-3">{error}</p>
      ...
      <div className="space-y-3">
          ...
          <Select
            ...
            onChange={(e) => {
              ...
              setForm({
                ...
                price: prof?.hourlyRate ?? form.price,
                currency: prof?.currency ?? form.currency,
              })
            }}
          >
            {professors.map((p) => (
              <option key={p.id} value={p.id}>
                {p.name} {p.isExternal ? t('professorExternalSuffix') : t('professorClubSuffix')}
              </option>
            ))}
          </Select>
        </div>
        <div>
          <label className="block text-sm font-semibold text-ink-700 mb-1">{t('courtLabel')}</label>
          <Select
            value={form.courtId}
        ...
        <div>
          <label className="block text-sm font-semibold text-ink-700 mb-1">{t('notesLabel')}</label>
...
function AddStudentModal({
  ...
  async function handleSave() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        setError(data.error ?? t('errorAddStudent'))
        return
      }
      onSaved(data.data)
    } catch {
  ...
  return (
    <Modal open onClose={onClose} title={t('addStudentTitle')} maxWidth="sm">
      <p className="text-xs text-ink-400 mb-3">
        {slot.professor.name} · {slot.date} · {slot.startTime} ·{' '}
      ...
      <div className="mb-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            type="button"
            onClick={() => setPayNow('pending')}
            className={`py-2 rounded-xl text-xs font-semibold border ${payNow === 'pending' ? 'bg-ink-100 border-ink-400 text-ink-700' : 'border-ink-200 text-ink-400'}`}
          >
            {t('paymentPending')}
          </button>
          <button
