...
function bucketize(
  ...
  if (byDay.length <= maxPoints) {
    return byDay.map((d) => ({
      label: new Date(d.date + 'T12:00:00').toLocaleDateString(locale, { day: 'numeric', month: 'short' }),
      bookings: d.bookings,
      revenue:  d.revenue,
    }))
  }
  ...
  for (let i = 0; i < byDay.length; i += size) {
    ...
    result.push({
      label:    new Date(bucket[0].date + 'T12:00:00').toLocaleDateString(locale, { day: 'numeric', month: 'short' }),
...
function HBarChart({ data, label, unit = '' }: {
  data: { label: string; value: number; max: number; color: string }[]
  label: string; unit?: string
}) {
  return (
    <Card className="p-6">
      <h3 className="text-sm font-bold text-gray-700 mb-5">{label}</h3>
      <div className="space-y-3">
        {data.map((d) => (
          <div key={d.label}>
            <div className="flex items-center justify-between text-xs mb-1.5">
              <span className="text-gray-600 font-medium truncate pr-2">{d.label}</span>
              <span className="text-gray-900 font-bold shrink-0">{unit}{d.value.toLocaleString()}</span>
            </div>
            <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${d.color}`} style={{ width: `${Math.min((d.value / d.max) * 100, 100)}%` }} />
            </div>
          </div>
        ))}
      </div>
...
function LineChart({ data, label, color = 'emerald', valueFormatter = (v: number) => String(v) }: {
  data: { label: string; value: number }[]
  label: string; color?: string; valueFormatter?: (v: number) => string
}) {
  ...
  if (data.length < 2) {
    return (
      <Card className="p-6">
        <h3 className="text-sm font-bold text-gray-700 mb-5">{label}</h3>
        <div className="h-36 flex items-center justify-center text-sm text-gray-400">{t('noDataChart')}</div>
      </Card>
    )
  }
    ...
    <Card className="p-6">
      ...
      <div className="relative h-36">
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full">
          ...
          {points.map((p, i) => (
            <circle key={i} cx={p.x} cy={p.y} r="1.5" fill={stroke} vectorEffect="non-scaling-stroke" />
          ))}
        </svg>
      </div>
      <div className="flex justify-between text-[10px] text-gray-400 mt-2 overflow-hidden">
        {data.map((d, i) => (
          <span key={i} className={i % tickEvery !== 0 ? 'opacity-0' : ''}>{d.label}</span>
        ))}
      </div>
    </Card>
  )
...
type StatsData = {
  ...
  byDay: { date: string; bookings: number; revenue: number }[]
  courtOccupancy: { id: string; name: string; sport: string; pct: number; slotsToday: number; bookedToday: number }[]
...
export default function EstadisticasPage() {
  ...
  const [activeClub,  setActiveClub]  = useState<{ id: string; name: string; currency: string } | null>(null)
  ...
  useEffect(() => {
    function readClub() {
      try {
        ...
        if (club?.id) setActiveClub({ id: club.id, name: club.name, currency: club.currency || 'USD' })
      } catch { /* ignore */ }
    }
    readClub()
  ...
  useEffect(() => {
    ...
    if (scope === 'club') {
      if (!activeClub) { setLoading(false); return }
    ...
    fetch(`${GW}/api/clubs/admin/${userId}`)
      ...
      .then(async (d) => {
        const clubs = d.data ?? []
        setAllClubs(clubs)
        if (clubs.length === 0) { setStats(null); setLoading(false); return }
        ...
        if (valid.length === 0) { setStats(null); setLoading(false); return }
  ...
  const courtData = (stats?.courtOccupancy ?? []).map((c) => ({
    ...
    value: c.pct,
    max:   maxOcc,
    color: c.pct >= 80 ? 'bg-emerald-500' : c.pct >= 50 ? 'bg-amber-400' : 'bg-gray-300',
  }))

  const trendIngresos    = stats ? pctTrend(stats.totalRevenue, stats.previousPeriod.totalRevenue) : undefined
  const trendConfirmadas = stats ? pctTrend(stats.confirmedBookings, stats.previousPeriod.confirmedBookings) : undefined
  const trendOcupacion   = stats ? pctTrend(stats.avgOccupancyToday, stats.avgOccupancyYesterday) : undefined
  const trendCanceladas  = stats ? pctTrend(stats.cancelledBookings, stats.previousPeriod.cancelledBookings) : undefined

  const currency = stats?.currency || activeClub?.currency || 'USD'
  const scopeLabel = scope === 'club'
    ...
    <div className="space-y-6 max-w-7xl">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${scope === 'club' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
            >
              {t('scopeThisClub')}
            </button>
            <button
              onClick={() => setScope('all')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${scope === 'all' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
            >
              {t('scopeAllClubs')}
            </button>
          </div>
          ...
          <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
            {PERIOD_VALUES.map((value) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${period === value ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
              >
                {t('periodDays', { value })}
              </button>
            ))}
      ...
      {loading ? (
        ...
        <Card className="py-20 text-center text-gray-400 text-sm">
          {scope === 'club' && !activeClub
            ? t('emptyStateNoActiveClub')
            : t('emptyStateNoData')}
        </Card>
      ) : (
        <>
