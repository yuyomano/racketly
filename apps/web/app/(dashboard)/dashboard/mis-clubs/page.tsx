...
import { Trash2, Power, BarChart3, Plus, Loader2, MapPin, Globe, Mail, Phone, Pencil, X, Check, Clock, Copy } from 'lucide-react'
...
function EditClubModal({ club, onClose, onSaved }: {
  club: Club; onClose: () => void; onSaved: (updated: Club) => void
}) {
  ...
  return (
    <Modal open onClose={onClose} title={t('title')} maxWidth="lg">
      <div className="space-y-4">
          ...
          {/* Deportes */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-2">{t('sports')}</label>
          ...
          {/* Amenidades */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-2">{t('amenities')}</label>
              ...
              {AMENITY_IDS.map((id) => {
                ...
                return (
                  <button key={id} type="button" onClick={() => toggleAmenity(id)}
                    ...
                    <span className={`w-4 h-4 rounded flex items-center justify-center shrink-0 ${checked ? 'bg-emerald-500' : 'bg-gray-100'}`}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1">{t('cancellationPolicy')}</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1">{t('bookingHorizon')}</label>
              <Select value={form.bookingHorizonDays}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">{t('slotGeneration')}</label>
            <Select value={form.slotGenerationHour}
              onChange={(e) => setForm({ ...form, slotGenerationHour: Number(e.target.value) })}>
              {Array.from({ length: 24 }, (_, h) => (
                <option key={h} value={h}>{String(h).padStart(2, '0')}:00</option>
              ))}
            </Select>
            <p className="text-[11px] text-gray-400 mt-1">{t('slotGenerationHint')}</p>
          </div>
          ...
          <div>
            ...
            <Input value={form.currency} onChange={(e) => setForm({ ...form, currency: e.target.value.toUpperCase() })}
              placeholder={t('currencyPlaceholder')} maxLength={3} />
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">{t('contactEmail')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1">{t('website')}</label>
...
function PeakHourTimeSelect({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    ...
    >
      {PEAK_TIME_OPTIONS.map((t) => <option key={t} value={t}>{t}</option>)}
    </select>
  )
}

function PeakHoursModal({ club, onClose, onSaved }: {
  club: Club; onClose: () => void; onSaved: (msg: string) => void
}) {
  ...
  useEffect(() => {
    fetch(`${GW}/api/clubs/${club.id}/peak-hours`)
      ...
      .then((d) => {
        ...
        if (rules.length === 0) {
          for (let dow = 0; dow <= 6; dow++) grouped[dow] = DEFAULT_PEAK_WINDOWS.map((w) => ({ ...w }))
        } else {
          for (const r of rules) grouped[r.dayOfWeek].push({ startTime: r.startTime, endTime: r.endTime })
        }
        setByDay(grouped)
      })
  ...
  function addWindow(dow: number) {
    setByDay((prev) => ({ ...prev, [dow]: [...(prev[dow] ?? []), { startTime: '18:00', endTime: '22:00' }] }))
  }
  ...
  async function handleSave() {
    for (const dow of Object.keys(byDay).map(Number)) {
      for (const w of byDay[dow]) {
        if (w.startTime >= w.endTime) {
          ...
          const dayLabel = dayKey === 'monday' ? t('monday')
            : dayKey === 'tuesday' ? t('tuesday')
            : dayKey === 'wednesday' ? t('wednesday')
            : dayKey === 'thursday' ? t('thursday')
            : dayKey === 'friday' ? t('friday')
            : dayKey === 'saturday' ? t('saturday')
            : dayKey === 'sunday' ? t('sunday')
            : ''
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setError(data.error ?? t('genericError')); return }
      const slotsUpdated = data.slotsUpdated ? t('savedWithSlots', { count: data.slotsUpdated }) : ''
  ...
  return (
    ...
    >
        <div className="flex items-center justify-between pb-4 mb-1 border-b border-gray-100 -mt-2">
          <div>
            <h2 className="font-black text-gray-900 flex items-center gap-2"><Clock className="w-4 h-4 text-emerald-500" /> {t('title')}</h2>
            <p className="text-xs text-gray-400 mt-0.5">{t('subtitle', { clubName: club.name })}</p>
          </div>
        </div>
        ...
        <div className="space-y-3 max-h-[65vh] overflow-y-auto">
          ...
          {loading ? (
            ...
            DAY_KEYS.map(({ dow, key }) => (
              <div key={dow} className="border border-gray-100 rounded-xl p-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-semibold text-gray-700">{t(key as 'monday')}</span>
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => copyToAllDays(dow)}
                      title={t('copyToAllTooltip')}
                      className="flex items-center gap-1 text-[11px] font-semibold text-gray-400 hover:text-emerald-600">
                      <Copy className="w-3 h-3" /> {t('copyToAll')}
                    </button>
                    <button type="button" onClick={() => addWindow(dow)}
                ...
                {(byDay[dow] ?? []).length === 0 ? (
                  <p className="text-xs text-gray-400">{t('noSchedule')}</p>
                ) : (
                  <div className="space-y-1.5">
                    {byDay[dow].map((w, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        <PeakHourTimeSelect value={w.startTime} onChange={(v) => updateWindow(dow, idx, 'startTime', v)} />
                        <span className="text-gray-300 text-xs">–</span>
                        <PeakHourTimeSelect value={w.endTime} onChange={(v) => updateWindow(dow, idx, 'endTime', v)} />
                        <button type="button" onClick={() => removeWindow(dow, idx)}
...
function ConfirmDialog({ msg, onConfirm, onCancel }: {
  ...
  return (
    <Modal open onClose={onCancel} maxWidth="sm">
      <p className="text-gray-800 font-semibold mb-5">{msg}</p>
      <div className="flex gap-3">
        <Button variant="secondary" onClick={onCancel} className="flex-1">{t('cancel')}</Button>
        <Button onClick={onConfirm} className="flex-1 !bg-red-600 hover:!bg-red-700">{t('confirm')}</Button>
      </div>
    </Modal>
  )
}

function ClubCard({ club, onToggle, onDelete, onEdit }: {
  club: Club
  ...
  async function handleToggle() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { showToast(data.error ?? t('toastErrorGeneric'), false); return }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { showToast(data.error ?? t('toastDeleteError'), false); return }
      onDelete(club.id)
    } catch {
      showToast(t('toastConnectionError'), false)
    } finally {
    ...
    <>
      ...
      {showEdit && (
        <EditClubModal
          ...
          onSaved={(updated) => { onEdit(updated); setShowEdit(false); showToast(t('toastUpdated'), true) }}
        />
      )}
      {showPeak && (
        <PeakHoursModal
          ...
          onSaved={(msg) => { setShowPeak(false); showToast(msg, true) }}
        />
      )}

      <Card className={`transition-all ${!club.isActive ? 'opacity-60 ring-1 ring-gray-200' : ''}`}>
        <div className="p-5">
          {/* Header */}
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-black text-gray-900 text-lg leading-tight">{club.name}</h3>
                {!club.isActive && (
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0 ${
                club.isActive ? 'bg-emerald-500' : 'bg-gray-300'
              } disabled:opacity-50`}
            >
          ...
          {(club.contactEmail || club.phone || club.website) && (
            <div className="flex items-center gap-3 text-xs text-gray-400 mb-4">
              {club.contactEmail && <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{club.contactEmail}</span>}
              {club.phone        && <span className="flex items-center gap-1"><Phone className="w-3 h-3" />{club.phone}</span>}
              {club.website      && <span className="flex items-center gap-1"><Globe className="w-3 h-3" />{club.website}</span>}
            </div>
          )}

          {toast && (
            <div className={`text-xs px-3 py-2 rounded-lg mb-3 ${toast.ok ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
              {toast.msg}
            </div>
          )}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-red-100 text-xs font-semibold text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors disabled:opacity-40"
            >
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        ...
        <Link href="/dashboard/nuevo-club">
          <Button><Plus className="w-4 h-4" /> {t('newClub')}</Button>
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            ...
            action={
              <Link href="/dashboard/nuevo-club">
                <Button size="sm"><Plus className="w-3.5 h-3.5" /> {t('emptyAction')}</Button>
              </Link>
            }
          />
        </Card>
        ...
        <>
          {active.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('sectionActive')}</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {active.map((c) => (
                  <ClubCard key={c.id} club={c} onToggle={handleToggle} onDelete={handleDelete} onEdit={handleEdit} />
                ))}
              </div>
            </section>
          )}
          ...
          {inactive.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('sectionInactive')}</h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {inactive.map((c) => (
                  <ClubCard key={c.id} club={c} onToggle={handleToggle} onDelete={handleDelete} onEdit={handleEdit} />
                ))}
