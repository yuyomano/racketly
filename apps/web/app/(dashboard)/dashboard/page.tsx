...
import { CalendarCheck, Wallet, Gauge, Users, Medal, ArrowUpRight, RefreshCw, Pencil } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mt-1.5">
        {data.map((_, i) => (
          <span key={i} className="flex-1 text-center text-[10px] font-medium text-gray-400">{days[i % 7]}</span>
        ))}
      </div>
    </div>
  )
    ...
    <div>
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <div className="flex items-start gap-1.5 min-w-0">
          <span className="mt-px shrink-0">{sport === 'padel' ? <PadelIcon size={12} /> : <PickleballIcon size={12} />}</span>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (parsed?.id) { setClubId(parsed.id); fetchStats(parsed.id); fetchClassesToday(parsed.id) }
      } catch { /* ignore */ }
    } else {
      setLoading(false)
    }
    ...
    function onClubChanged(e: Event) {
      ...
      if (detail?.id) { setClubId(detail.id); fetchStats(detail.id); fetchClassesToday(detail.id) }
    }
  ...
  const confirmedToday = (stats?.todayBookings?.filter((b: any) => b.status === 'confirmed').length ?? 0)
    + (stats?.confirmedClassesToday ?? 0)
  const totalSlotsToday = stats?.courtOccupancy?.reduce((s: number, c: any) => s + c.slotsToday, 0) ?? 0

  const avgOccupancyToday = stats?.courtOccupancy?.length > 0
    ? Math.round(stats.courtOccupancy.reduce((s: number, c: any) => s + c.pct, 0) / stats.courtOccupancy.length)
    : 0

  const trendReservasHoy   = stats ? pctTrend(confirmedToday, stats.confirmedYesterday) : undefined
  const trendIngresos      = stats ? pctTrend(stats.totalRevenue, stats.previousPeriod?.totalRevenue ?? 0) : undefined
  const trendOcupacion     = stats ? pctTrend(avgOccupancyToday, stats.avgOccupancyYesterday) : undefined
  const trendReservasTotal = stats ? pctTrend(stats.totalBookings, stats.previousPeriod?.totalBookings ?? 0) : undefined

  // Lista unificada de la agenda de hoy: reservas de cancha + clases, ordenadas por hora.
  const todayAgenda = [
    ...(stats?.todayBookings ?? []).map((b: any) => ({ type: 'booking' as const, time: b.slot?.startTime ?? '', data: b })),
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            sub={t('statSubCanceladasPendientes', { cancelled: stats.cancelledBookings, pending: stats.pendingBookings })}
            icon={Users}
            tone="gray"
      ...
      {stats && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Card>
            <CardHeader><CardTitle>{t('ocupacionPorPistaHoy')}</CardTitle></CardHeader>
          ...
          <Card className="lg:col-span-2">
            <CardHeader className="flex items-center justify-between">
              ...
              <a href="/dashboard/reservas" className="text-xs text-emerald-600 font-semibold hover:underline flex items-center gap-1">
            ...
            <CardBody className="space-y-2">
              {todayAgenda.length > 0 ? todayAgenda.map((item) => {
                if (item.type === 'class') {
                  const s = item.data
                  const activeStudents = (s.bookings ?? []).filter((bk: any) => bk.status === 'active')
                  const studentNames = activeStudents.map((bk: any) => bk.studentName).join(', ') || t('sinAlumnosAun')
                  return (
                    <div key={`class-${s.id}`} className="flex items-start gap-4 p-3 rounded-xl bg-violet-50 hover:bg-violet-100 transition-colors">
                      <span className="text-sm font-mono font-bold text-violet-700 w-12 shrink-0 mt-0.5">
                        {s.startTime}
                      </span>
                      <span className="mt-0.5 shrink-0">🎓</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-900 break-words">{t('claseCon', { name: s.professor?.name ?? '' })}{s.court ? ` · ${s.court.name}` : ''}</p>
                ...
                const b = item.data
                const names = Array.isArray(b.players) && b.players.length > 0
                  ? b.players.map((p: any) => p.name).join(', ')
                  : b.userId
                const meta = bookingStatusMeta(b.status)
                return (
                <div key={b.id} className="flex items-start gap-4 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <span className="text-sm font-mono font-bold text-gray-700 w-12 shrink-0 mt-0.5">
                    {b.slot?.startTime ?? '—'}
                  </span>
                  <span className="mt-0.5 shrink-0">{b.slot?.court?.sport === 'padel' ? <PadelIcon size={16} /> : <PickleballIcon size={16} />}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 break-words">{b.slot?.court?.name}</p>
        ...
        <Card>
          ...
          <CardBody>
            <div className="grid grid-cols-3 gap-6 text-center">
              ...
              <div>
                <p className="text-2xl font-black text-violet-600">{formatCurrency(stats.totalRevenue, currency)}</p>
                {currency !== 'USD' && (
                  <p className="text-xs text-gray-400 mt-0.5">≈ {formatCurrency(toUSD(stats.totalRevenue, currency), 'USD')} USD</p>
                )}
