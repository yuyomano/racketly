...
function AcceptContent() {
  ...
  useEffect(() => {
    if (!token) { setStatus('error'); setMessage('No se encontró un token de invitación en el enlace.'); return }
    fetch(`${GW}/api/auth/invite/${token}`)
      .then(async (r) => {
        const data = await r.json()
        if (!r.ok) { setStatus('error'); setMessage(data.error || 'Invitación no válida'); return }
        setInvite(data.data)
        setStatus('ready')
      })
      .catch(() => { setStatus('error'); setMessage('Error de conexión con el servidor') })
  }, [token])
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!res.ok) { setFormError(data.error || 'No se pudo activar la cuenta'); return }
      setStatus('done')
  ...
  if (status === 'error') {
    return (
      <div className="text-center space-y-3">
        ...
        <Button onClick={() => router.push('/login')} className="mt-2">Ir al login</Button>
      </div>
    )
  }
  ...
  if (status === 'done') {
    return (
      <div className="text-center space-y-3">
        ...
        <p className="text-gray-500 text-sm">Ya puedes iniciar sesión con tu email y tu nueva contraseña.</p>
        <Button onClick={() => router.push('/login')} className="mt-4">Ir al login</Button>
      </div>
    )
  }
    ...
    <div className="space-y-5">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Te crearon una cuenta con <span className="font-semibold">{invite!.email}</span>. Define tu contraseña para activarla.
        </p>
      </div>
