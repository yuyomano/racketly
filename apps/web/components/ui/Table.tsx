...
export function TableHead({ className, children, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
  return (
    <thead className={cn('text-left text-xs text-gray-400 uppercase tracking-wider bg-gray-50', className)} {...props}>
      {children}
    </thead>
  )
}
...
export function TableSpanRow({ colSpan, className, children }: { colSpan: number; className?: string; children: React.ReactNode }) {
  return (
    <tr>
