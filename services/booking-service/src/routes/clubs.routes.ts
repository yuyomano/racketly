...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (term) {
      ...
      clubs = clubs.filter(
        (c) => normalizeText(c.name).includes(needle) || normalizeText(c.city).includes(needle)
      )
    }
    ...
    try {
      ...
      const [
        ...
        prisma.classSlot.findMany({
          ...
          select: {
            courtId: true,
            bookings: { where: { status: 'active' }, select: { id: true } },
          },
        }),
        prisma.classSlot.findMany({
      ...
      const prevConfirmed = prevBookings.filter(
        (b) => b.status === 'confirmed' || b.status === 'completed'
      )
      ...
      confirmed.forEach((b) => {
        ...
        if (byDay[day]) {
          byDay[day].bookings++
          byDay[day].revenue += b.amountPaid ?? 0
        }
      })
      ...
      const confirmedClassesYesterday = yesterdayClassSlots.filter(
        (cs) => cs.bookings.length > 0
      ).length
    ...
    try {
      ...
      const [bookings, classBookings, tournamentParticipants] = await Promise.all([
        ...
        prisma.tournamentParticipant.findMany({
          ...
          include: {
            tournament: { select: { name: true, currency: true, sport: true, startDate: true } },
...
router.put('/:id', requireClubAccess, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const {
      ...
      cancellationPolicy,
      timezone,
      paymentWarningMinutesBefore,
      rosterWarningHoursBefore,
    } = req.body
    ...
    if (timezone !== undefined) {
      ...
      try {
        ...
        throw new AppError(
          'timezone debe ser un identificador IANA válido (ej. "America/Santo_Domingo")',
          400
        )
      }
      data.timezone = tz
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (const r of rules) {
        ...
        if (!timeRe.test(r.startTime) || !validEnd)
          throw new AppError('Horas inválidas (formato HH:mm)', 400)
        if (r.startTime >= r.endTime)
          throw new AppError('La hora de inicio debe ser menor a la de fin', 400)
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const ca = await prisma.clubAdmin.upsert({
        ...
        include: {
          user: { include: { playerProfile: { select: { displayName: true, avatarUrl: true } } } },
        },
      })
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!ca || ca.clubId !== req.params.id)
        throw new AppError('Admin no encontrado en este club', 404)
