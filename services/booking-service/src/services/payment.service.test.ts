...
describe('payment.service (DEV_MODE, sin STRIPE_SECRET_KEY)', () => {
  ...
  it('createStripePaymentIntent devuelve un clientSecret simulado sin llamar a Stripe', async () => {
    ...
    const result = await createStripePaymentIntent({
      amount: 5000, currency: 'DOP', bookingId: 'booking-1', userId: 'user-1', description: 'test',
    })
  ...
  it('constructStripeEvent lanza si Stripe no está configurado (no se puede verificar un webhook sin secret real)', async () => {
    ...
    await expect(constructStripeEvent(Buffer.from('{}'), 'sig')).rejects.toThrow('Stripe no configurado')
  })
