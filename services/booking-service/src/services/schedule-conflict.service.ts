...
export async function hasConflictingClass(
  courtId: string, date: string, startMin: number, endMin: number, excludeSlotId?: string
): Promise<boolean> {
  const slots = await prisma.classSlot.findMany({
    where: {
      courtId, date, status: { not: 'cancelled' },
...
export async function hasConflictingMaintenance(
  courtId: string, date: string, startMin: number, endMin: number
): Promise<boolean> {
