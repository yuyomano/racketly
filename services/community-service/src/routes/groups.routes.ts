...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const groups = await prisma.group.findMany({ where, orderBy: { memberCount: 'desc' }, take: 50 })
...
router.post('/:id/join', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await prisma.group.update({ where: { id: req.params.id }, data: { memberCount: { increment: 1 } } })
