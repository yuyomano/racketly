...
new Worker('push-notifications', async (job) => {
  ...
  if (!token) { console.info(`[Push] SKIP (sin token): ${title}`); return }
...
new Worker('email-notifications', async (job) => {
  ...
  if (!sendgridReady) { console.info(`[Email] SKIP (SendGrid no config): ${subject} → ${to}`); return }
  await sgMail.send({ to, from: { email: 'noreply@racketly.app', name: 'Racketly' }, subject, html, text })
  console.info(`[Email] Sent to ${to}: ${subject}`)
}, { connection: redisConnection })

new Worker('smart-alerts', async (job) => {
