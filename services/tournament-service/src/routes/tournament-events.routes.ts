...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { clubId, organizerId, name, description, startDate, endDate, schedulingWindows } =
      req.body
...
router.post('/:id/schedule', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (pending.length === 0)
      throw new AppError('No hay partidos pendientes de horario en los torneos de este evento', 400)
