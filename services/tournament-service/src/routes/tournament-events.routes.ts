...
import {
  matchFormatDurationMinutes, resolveMatchFormat, knockoutStageKeyForRound,
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const events = await prisma.tournamentEvent.findMany({
      ...
      include: { tournaments: { select: { id: true, name: true, category: true, genderCategory: true, status: true, sport: true } } },
    })
...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { clubId, organizerId, name, description, startDate, endDate, schedulingWindows } = req.body
    if (!clubId) throw new AppError('clubId es requerido', 400)
    if (!organizerId) throw new AppError('organizerId es requerido', 400)
    ...
    if (new Date(endDate) < new Date(startDate)) throw new AppError('endDate no puede ser anterior a startDate', 400)
    ...
    const event = await prisma.tournamentEvent.create({
      data: {
        clubId, organizerId, name: name.trim(), description,
  ...
  try {
    ...
    if (schedulingWindows !== undefined) {
      if (schedulingWindows === null) {
        ...
        if (!sanitized) throw new AppError('schedulingWindows inválido — cada franja necesita date (YYYY-MM-DD), openTime y closeTime (HH:MM) con closeTime > openTime', 400)
...
router.delete('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (event._count.tournaments > 0) {
      throw new AppError('No se puede eliminar un evento con torneos asociados — desvincúlalos primero', 400)
    }
...
router.get('/:id/matches', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const tournamentCtx = new Map<string, { baseFormat: string; overrides: MatchFormatOverrides | null; maxKnockoutRound: number }>()
...
router.post('/:id/schedule', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!event) throw new AppError('Evento no encontrado', 404)
    if (event.tournaments.length === 0) throw new AppError('El evento no tiene torneos asociados', 400)

    const sports = new Set(event.tournaments.map((t) => t.sport))
    const resolvedSport = sport || (sports.size === 1 ? [...sports][0] : null)
    if (!resolvedSport) {
      throw new AppError('El evento mezcla deportes distintos — pasa "sport" explícito para elegir qué pistas usar', 400)
    }
    ...
    const tournamentCtx = new Map<string, { baseFormat: string; overrides: MatchFormatOverrides | null; restMs: number; maxKnockoutRound: number }>()
    for (const t of event.tournaments) {
      const maxRoundAgg = await prisma.match.aggregate({
        where: { tournamentId: t.id, stage: 'knockout' },
        _max: { round: true },
    ...
    function formatForMatch(tournamentId: string, m: { stage: string; round: number | null }): string {
      const ctx = tournamentCtx.get(tournamentId)!
      const stageKey = m.stage === 'knockout' && m.round !== null
        ? knockoutStageKeyForRound(m.round, ctx.maxKnockoutRound)
        : null
      return resolveMatchFormat({ matchFormat: ctx.baseFormat as never, matchFormatOverrides: ctx.overrides }, stageKey)
    }
    function durationMsForMatch(tournamentId: string, m: { stage: string; round: number | null }): number {
      return matchFormatDurationMinutes(formatForMatch(tournamentId, m)) * 60000
    }

    const pending = await prisma.match.findMany({
      where: { tournamentId: { in: event.tournaments.map((t) => t.id) }, scheduledAt: null, status: 'scheduled', round: { not: null } },
      orderBy: [{ round: 'asc' }, { tournamentId: 'asc' }, { id: 'asc' }],
    })
    if (pending.length === 0) throw new AppError('No hay partidos pendientes de horario en los torneos de este evento', 400)
    ...
    const alreadyScheduled = await prisma.match.findMany({
      where: { tournamentId: { in: eventTournamentIds }, scheduledAt: { not: null }, player1Id: { not: null }, player2Id: { not: null } },
      select: { tournamentId: true, player1Id: true, player1PartnerId: true, player2Id: true, player2PartnerId: true, scheduledAt: true, stage: true, round: true },
    })
    ...
    function peekEarliestCourt(notBefore: number, durationMs: number): { courtId: string; start: number } {
      let bestCourt = courts[0].id
      let bestFreeAt = Infinity
      for (const [courtId, freeAt] of courtFreeAt) {
        if (freeAt < bestFreeAt) { bestFreeAt = freeAt; bestCourt = courtId }
      }
      let start = Math.max(notBefore, bestFreeAt)
      if (windows) {
        start = nextPlayable(start, windows)
        if (start === Infinity) throw new AppError('No hay franjas horarias suficientes en el evento para agendar todos los partidos pendientes', 400)
        const win = windows.find((w) => start >= w.start && start < w.end)!
        if (start + durationMs > win.end) {
          const idx = windows.findIndex((w) => w.start > start)
          if (idx === -1) throw new AppError('No hay franjas horarias suficientes en el evento para agendar todos los partidos pendientes', 400)
    ...
    for (const m of known) {
      ...
      tournamentFloor.set(m.tournamentId!, Math.max(tournamentFloor.get(m.tournamentId!) ?? 0, start + durationMs + ctx.restMs))
    }
    ...
    const orderedWaves = [...waveGroups.values()].sort((a, b) => (a[0].round ?? 0) - (b[0].round ?? 0))
    ...
    const byTournament = event.tournaments.map((t) => {
      const tMatches = updates.filter((u) => pending.find((p) => p.id === u.id)?.tournamentId === t.id)
      const lastEnd = tMatches.length
        ? Math.max(...tMatches.map((u) => u.scheduledAt.getTime() + durationMsForMatch(t.id, pending.find((p) => p.id === u.id)!)))
        : null
      return { tournamentId: t.id, name: t.name, scheduledMatches: tMatches.length, estimatedEndAt: lastEnd ? new Date(lastEnd) : null }
    })

    const estimatedEndAt = new Date(Math.max(...byTournament.filter((b) => b.estimatedEndAt).map((b) => b.estimatedEndAt!.getTime())))

    return res.json({
      success: true,
      data: { scheduledMatches: updates.length, courtsUsed: courts.length, estimatedEndAt, byTournament },
    })
  } catch (err) { return next(err) }
})
